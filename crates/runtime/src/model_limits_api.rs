...
mod tests {
    ...
    #[tokio::test]
    async fn custom_responses_discovery_requests_versioned_models_and_parses_ids() {
        let (base_url, server) = spawn_model_fixture(
            200,
        ...
        let mut config = NcaConfig::default();
        config.provider.default = ProviderKind::Custom;
        config.provider.custom.base_url = format!("{base_url}/responses/v1");
        ...
        let ids = fetch_provider_model_ids(&config).await;
        let request = server.join().expect("Responses model fixture thread");

        assert_eq!(ids, vec!["responses-alpha", "responses-zeta"]);
        assert!(request.starts_with("GET /responses/v1/models HTTP/1.1\r\n"));
        assert!(
            request
                .to_ascii_lowercase()
