...
use nca_core::tools::web_search::{ManualSearchClock, SearchClock, SearchLimiter, WebSearchTool};
...
async fn independent_sessions_share_duckduckgo_limiter_and_request_pacing() {
    ...
    assert!(
        second_start.saturating_sub(first_start) >= Duration::from_millis(20),
        "fake-clock request starts were only {:?} apart",
        second_start.saturating_sub(first_start)
    );
