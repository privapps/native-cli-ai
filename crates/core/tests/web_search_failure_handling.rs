...
#[tokio::test]
async fn concurrent_bing_requests_share_one_in_flight_slot() {
    ...
    assert_eq!(bing.log.lock().expect("Bing log").max_in_flight, 1);
}

#[tokio::test]
async fn concurrent_process_tools_share_the_bing_gate() {
    let mut first = response(BING_RESULT, 200);
    first.delay = Duration::from_millis(100);
    let mut second = response(BING_RESULT, 200);
    second.delay = Duration::from_millis(100);
...
async fn duckduckgo_challenge_records_status_and_cooldown_before_retry() {
    ...
    let result = tool
        .execute(&call(serde_json::json!({"query": "cooldown"})))
        .await;
    bing.handle.join().expect("Bing fixture thread");
    duckduckgo.handle.join().expect("DDG fixture thread");
    ...
    assert!(
        log.request_starts[1].duration_since(log.request_starts[0]) >= Duration::from_millis(4)
    );
}
...
async fn repeated_duckduckgo_challenges_use_bounded_shared_backoff() {
    ...
    let result = tool
        .execute(&call(serde_json::json!({"query": "backoff"})))
        .await;
    bing.handle.join().expect("Bing fixture thread");
    duckduckgo.handle.join().expect("DDG fixture thread");
    ...
    assert!(
        log.request_starts[2].duration_since(log.request_starts[1]) >= Duration::from_millis(9)
    );
    assert!(
        log.request_starts[3].duration_since(log.request_starts[2]) >= Duration::from_millis(14)
    );
}
...
async fn independent_sessions_share_duckduckgo_limiter_and_request_pacing() {
    ...
    assert_eq!(log.max_in_flight, 1);
    assert!(
        log.request_starts[1].duration_since(log.request_starts[0]) >= Duration::from_millis(18)
    );
}
