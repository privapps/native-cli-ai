...
fn request_keyboard_enhancement(out: &mut impl Write) -> io::Result<()> {
    match execute!(
        ...
        Ok(_) => Ok(()),
        // Crossterm reports this on terminals without keyboard enhancement
        // support, including the legacy Windows console. Modified-key
        // reporting is an enhancement, so retain the ordinary key path.
...
mod tests {
    ...
    #[test]
    fn keyboard_enhancement_is_requested_when_supported() {
        let mut output = Vec::new();

        request_keyboard_enhancement(&mut output).expect("request should be serialized");

        assert_eq!(output, b"\x1b[>1u");
    }

    #[test]
    fn unsupported_keyboard_enhancement_does_not_fail_setup() {
