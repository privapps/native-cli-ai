...
**Date:** 2026-07-29
**Related:** [Reasoning effort specification](../specs/2026-07-29-reasoning-effort.md)
**Last verified:** 2026-07-31 (documentation review)

## Goal

Add the global `model.reasoning_effort` setting to OpenAI-compatible Chat Completions and Responses requests, with the literal `nil` sentinel omitting the JSON property, then expose it through the CLI and TUI.

## Boundaries

- Shared OpenAI-compatible request-body construction is the primary wire seam.
- Existing provider request-capture tests observe OpenAI, OpenRouter, Custom Chat Completions, Custom Responses, and Anthropic-compatible behavior.
- Existing configuration, CLI, and TUI command boundaries cover user-facing configuration behavior.

## Bounded steps

1. [x] Extend model configuration with a string defaulting to `nil`; add failing request/config tests; implement trimming and conditional JSON emission for OpenAI, OpenRouter, Custom Chat Completions, and nested `reasoning.effort` for Custom Responses while leaving Anthropic-compatible requests unchanged.
...
- Existing thinking settings remain independent. Chat Completions and Anthropic temperature behavior is unchanged; Custom Responses preserves configured `temperature` and surfaces provider rejection normally.
