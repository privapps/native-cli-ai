...
24. As an `nca` user, I want temperature behavior unchanged on Chat Completions and Anthropic-compatible paths, while Responses preserves configured `temperature`, so that the provider receives the setting I selected.
...
- Keep `temperature` unchanged on Chat Completions, Anthropic-compatible paths, and Custom Responses. Surface a provider rejection normally; do not add capability detection or retry behavior.
