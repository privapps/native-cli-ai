...
**Date:** 2026-07-29
**Related:** [Custom Provider OpenAI Responses implementation plan](../plans/2026-07-29-custom-provider-responses-api.md)
...
22. As a user connecting a Responses model, I want nca to omit model-dependent `temperature`, so that models that reject that field can still receive valid requests.
...
- Map the existing output-token budget to `max_output_tokens` and omit `temperature` from Responses requests because model support varies. Do not infer model capabilities or automatically retry after provider rejection.
...
- Include a regression fixture where the gateway rejects any request containing `temperature`, and assert that the request succeeds without that field and without retrying.
