...
## Historical MVP Scope (Phase 1)

This section records the original phase boundary. Current shipped capabilities are described in the [user guide](../user/index.md) and [technical reference](../reference/index.md).
...
- Config via TOML (`~/.local/share/ncacli/config.toml` with legacy `~/.nca/config.toml` fallback, plus `.nca/config.local.toml`)
...
### Historical Out of Scope for MVP
...
MCP integration and image/vision input were originally deferred but are now supported capabilities. Refer to the user documentation for their current contracts.
...
## Historical Post-MVP Parity Roadmap
...
- durable memory and session summaries (partially shipped through workspace memory and context management)
