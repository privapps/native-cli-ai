...
This guide covers common `nca` CLI workflows and examples. The generated list of top-level commands and global options lives in the [CLI Reference](../reference/cli-reference.md).
...
nca spawn --prompt "document all public APIs" --model "MiniMax-M2.5"
```
...
Create and recover a durable autoresearch session, inspect its state, and
review persisted results without rerunning the program.

For the Markdown program format and agent-facing bounded continuation
contract, see [Autoresearch](./autoresearch.md).

```bash
nca autoresearch start --program <PROGRAM> [--workspace <PATH>] [--session-id <ID>] [--json]
nca autoresearch status [SESSION_ID] [--workspace <PATH>] [--json]
nca autoresearch stop [SESSION_ID] [--workspace <PATH>] [--json]
nca autoresearch resume [SESSION_ID] [--workspace <PATH>] [--json]
nca autoresearch results [SESSION_ID] [--workspace <PATH>] [--json]
nca autoresearch once <PROGRAM> [--workspace <PATH>]
```

| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--program` | path | required for `start` | Research program markdown file |
| `--workspace` | path | current directory | Workspace for research output |
| `--session-id` | string | generated | Stable id for a new session |
| `--json` | flag | human output | Emit the same lifecycle/result data as JSON |

Session metadata and append-only JSONL results are stored under
`.nca/autoresearch/sessions/`. `resume` is idempotent for an already-running
session, so restarting the CLI does not start duplicate execution.
