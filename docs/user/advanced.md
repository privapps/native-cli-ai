...
```toml
[memory]
file_path = ".nca/memory.json"    # Default sentinel; resolves under the product home
max_notes = 128                    # Maximum number of notes
```

### How Memory Works

- Notes are stored as JSON under the product home's workspace directory by default
...
- Notes are workspace-scoped (the `.nca/memory.json` sentinel resolves to the product home)
