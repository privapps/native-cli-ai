...
model = "my-model"
temperature = 0.7  # Sent to Responses requests as configured
```
...
- The configured `temperature` value is serialized unchanged. If a gateway or model rejects it, nca surfaces that provider error without rewriting the request or retrying with a different setting.
